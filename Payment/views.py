...
class PayHistoryView(APIView):
    @swagger_auto_schema(
        operation_id="카카오페이 결제 내역 조회 API (심화)",
        operation_description="""
        로그인한 사용자의 승인 완료된 결제 건마다 카카오페이 '주문 조회' API를 호출해
        상품 이름, 결제 금액, 결제 수단, 결제 승인 시간을 반환합니다.
        """,
    ...
    def get(self, request):
        ...
        for payment in payments:
            # 카카오페이 주문 조회 API 호출 (DB에 없는 결제 수단, 승인 시간 등을 가져옴)
            ...
            if response.status_code != 200:
                print(f"주문 조회 실패 (tid={payment.tid}): {response.text}")
                continue
            ...
            history.append({
                ...
                'amount': (order.get('amount') or {}).get('total'),
